...
# create the new string. This adds up to a complexity of O(M**2×N).

# Breadth first search in the worst case might go to each of the N words. For each word, we need to examine M possible 
# intermediate words/combinations. Notice, we have used the substring operation to find each of the combination. Thus, M combinations 
...
# Each word in the word list would have M intermediate combinations. To create the adj_map dictionary we save an intermediate word as 
# the key and its corresponding original words as the value. Note, for each of M intermediate words we save the original word of 
# length M. This simply means, for every word we would need a space of M**2 to save all the transformations corresponding to it. 
# Thus, adj_map would need a total space of O(M**2×N).
